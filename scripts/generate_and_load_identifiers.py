...
def process_and_load_identifiers(input_csv_path, connection_string, database_name):
    ...
    try:
        ...
        # Fails closed: anything other than an explicit yes aborts before we can
        # touch the wrong database.
        if input(f"Is the MongoDB database name '{database_name}' correct? (y/n): ").strip().lower() not in ("y", "yes"):
            logging.info("Aborted: database name not confirmed.")
            exit()
